...
sinDivisiones = int(input("¿Sin divisiones (1) o con divisiones (0)?: "))
maximoCoeficiente = 4
#######################################################################################
...
#        print(coeficientesRespuesta)
        if compruebaCoeficientes(coeficientesRespuesta,solucion):
            ejercicio = "$(" + escribePolinomioEnLaTeX(A.coef) + ")+(" + escribePolinomioEnLaTeX(B.coef) + ")$"
...
def generaOperacionesTipo3(numeroOperacionesDistintas, solucion, maximoPositivo):
    ...
    while contador < numeroOperacionesDistintas:
        ...
        coeficientesRespuesta = (A*B).coef
        if compruebaCoeficientes(coeficientesRespuesta,solucion):
            ejercicio = "$(" + escribePolinomioEnLaTeX(A.coef) + ")\cdot{}(" + escribePolinomioEnLaTeX(B.coef) + ")$"
            listaOperaciones.append(ejercicio)
            listaOperacionesUnicas = np.unique(listaOperaciones)
            contador = len(listaOperacionesUnicas)
    return listaOperacionesUnicas
...
def generaOperacionesTipo19(numeroOperacionesDistintas, solucion, maximoPositivo):
    ...
    while contador < numeroOperacionesDistintas:
        ...
        if compruebaCoeficientes(coeficientesRespuesta,solucion):
            if N < 0:
                ejercicio = "$" + str(M) + "\cdot{}(" + escribePolinomioEnLaTeX(A.coef) + ")+(" + str(N) + ")\cdot{}(" + escribePolinomioEnLaTeX(B.coef) + ")$"
            else:
                ejercicio = "$" + str(M) + "\cdot{}(" + escribePolinomioEnLaTeX(A.coef) + ")+" + str(N) + "\cdot{}(" + escribePolinomioEnLaTeX(B.coef) + ")$"
            listaOperaciones.append(ejercicio)
            listaOperacionesUnicas = np.unique(listaOperaciones)
...
for koko in range(len(elementos)):
    ...
    fLaTeX.write(r"Para desencriptarlo, tendr\'{a}s que resolver una serie de operaciones con polinomios. Cuando obtengas el polinomio resultante, \textbf{multiplica} el coeficiente de cada término de grado mayor o igual que 1 por su grado. Luego, \textbf{suma} los productos obtenidos y \textbf{añade} el término independiente. El resultado te indicará la letra."+"\n")
